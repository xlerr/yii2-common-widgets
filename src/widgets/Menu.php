...
class Menu extends \yii\widgets\Menu
{
    ...
    protected function isItemActive($item)
    {
        if (isset($item['url']) && is_array($item['url']) && isset($item['url'][0])) {
            ...
            if (count($item['url']) > 1) {
                unset($item['url'][0]);
                if (!$this->checkParams($item['url'], $this->params)) {
                    return false;
                }
            }
     ...
     * @return bool
     */
    protected function checkParams($args, $params)
    {
        foreach ($args as $name => $value) {
            ...
            if (is_array($value) && $this->checkParams($value, $params[$name])) {
                continue;
            } elseif ($params[$name] != $value) {
                return false;
            }
        }
